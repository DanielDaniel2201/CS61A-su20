def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [one.insert(0, item) for one in nested_list]
...
def make_glookup(class_assignments):
    ...
    """
    "*** YOUR CODE HERE ***"
    key_list, point_earned = {}, 0

    def helper(keyword, earned_points):
        nonlocal key_list, point_earned
        key_list[keyword] = class_assignments[keyword]
        point_earned += earned_points
        total_point = sum(key_list.values())
        return point_earned / total_point 
    return helper
