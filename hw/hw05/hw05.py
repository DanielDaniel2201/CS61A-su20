def make_bank(balance):
    ...
    def bank(message, amount):
        "*** YOUR CODE HERE ***"
        assert message == 'deposit' or message == 'withdraw', 'Invalid message'
        nonlocal balance
        if message == "deposit":
            balance += amount
            return balance
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, entering):
        nonlocal store, balance
        if entering == password:
            ...
            return "Too many incorrect attempts. Attempts: ", store
        else:
            store += [entering]
