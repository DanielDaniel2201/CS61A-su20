...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    pig_out = False
    dice_nums = [dice() for i in num_rolls]
    for num in dice_nums:
        if num == 1:
            pig_out = True
    return 1 if pig_out else sum(dice_nums)
    # END PROBLEM 1
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    return player_score % 10 - opponent_score % 10 == opponent_score // 10
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    ma = make_averaged(dice, trials_count)
    trial = [ma(i, dice) for i in range(1, 11)]
    return trial.index(max(trial)) + 1
    # END PROBLEM 9
