...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand = rest.map(lambda opr: scheme_eval(opr, env))
    ...
    def apply(self, args, env):
        ...
        def convert(scm_lst):
            if scm_lst is nil:
                ...
                return [scm_lst.first] + convert(scm_lst.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(symbol, LambdaProcedure(formals, expressions.rest, env))
        return symbol
        # END PROBLEM 9
    else:
    ...
    if not expressions:
        ...
        while expressions:
            ...
            if is_true_primitive(value):
                return value
            elif is_false_primitive(value) and not expressions.rest:
                return False
            expressions = expressions.rest
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        value, name = scheme_eval(bindings.first.rest.first, env), bindings.first.first
