...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place < self.min_range and place != beehive:
            place = place.entrance
            curr_place += 1

        while curr_place <= self.max_range and place != beehive:
            if place.bees:
...
class FireAnt(Ant):
    ...
    damage = 3
    food_cost = 5
    armor = 3   # OVERRIDE CLASS ATTRIBUTES HERE
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            if self.place.bees:
                rb = rANTdom_else_none(self.place.bees)
                self.eat_bee(rb)
        # END Problem 6
